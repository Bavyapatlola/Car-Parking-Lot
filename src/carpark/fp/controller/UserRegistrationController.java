...
import carpark.fp.dao.LoginDAO;
import carpark.fp.dao.UserRegistrationDAO;
    ...
    public void CheckUserName(KeyEvent ev) {
        if (getRole() == 'G' || getLoginUser().getRole() == 'A') {
            ...
            if (!Common.IsNullOrEmpty(txtUserName.getText()))
                if (!IsUserNameAvailable(txtUserName.getText()))
        ...
        if (checkErrorMessage()) {
            ...
            if ( getRole()=='G' || getLoginUser().getRole() == 'A') {
                ...
                if (result > 0) {
                    ...
                    errEmailAddress.setText(res+"Clear All Error Message");
