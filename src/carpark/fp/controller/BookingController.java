...
import carpark.fp.dao.AdminAccessDAO;
import carpark.fp.dao.ParkingDAO;
import carpark.fp.dao.Interface.IAdminAccessDAO;
import carpark.fp.dao.Interface.IParkingDAO;
...
public class BookingController extends BaseController {
    ...
    public void BookParking_Click() {
        ...
        if (Common.IsNullOrEmpty(cbeParkingSpace.getSelectionModel().getSelectedItem())) {
            ...
            LocalDate bookingEndDate = toDate.getValue();
            LocalDate bookingStartDate=LocalDate.now(clock);
            long numberOfDays= ChronoUnit.DAYS.between(bookingStartDate,bookingEndDate);
            model.setToDate(Date.from(bookingEndDate.atStartOfDay(ZoneId.systemDefault()).toInstant()));
            model.setPrice(100*(int)numberOfDays);
